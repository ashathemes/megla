<?php
/**
 * Megla functions and definitions
 *
...
if ( ! function_exists( 'megla_setup' ) ) :
	 ...
	 */
	function megla_setup() {
		/*
		 * Make theme available for translation.
		 * Translations can be filed in the /languages/ directory.
		 * If you're building a theme based on megla, use a find and replace
		 * to change 'megla' to the name of your theme in all the template files.
		 */
		load_theme_textdomain( 'megla', get_template_directory() . '/languages' );
		...
		register_nav_menus(
			array(
				'menu-1' => esc_html__( 'Primary', 'megla' ),
			)
		);
...
add_action( 'after_setup_theme', 'megla_setup' );
 ...
 */
function megla_content_width() {
	// This variable is intended to be overruled from themes.
	// Open WPCS issue: {@link https://github.com/WordPress-Coding-Standards/WordPress-Coding-Standards/issues/1043}.
	// phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedVariableFound
	$GLOBALS['content_width'] = apply_filters( 'megla_content_width', 640 );
}
add_action( 'after_setup_theme', 'megla_content_width', 0 );
 ...
 */
function megla_widgets_init() {
	register_sidebar(
		array(
			'name'          => esc_html__( 'Sidebar', 'megla' ),
			'id'            => 'sidebar-1',
			'description'   => esc_html__( 'Add widgets here.', 'megla' ),
...
add_action( 'widgets_init', 'megla_widgets_init' );

/**
 * Register custom fonts.
 */
function megla_fonts_url() {
	$fonts_url = '';
 ...
 */
function megla_scripts() {
	wp_enqueue_style( 'megla-google-fonts', megla_fonts_url(), array(), null );
	...
	wp_enqueue_script( 'megla-script', get_template_directory_uri() . '/assets/js/megla-script.js', array('jquery'), MEGLA_VERSION, true );

	if ( is_singular() && comments_open() && get_option( 'thread_comments' ) ) {
		wp_enqueue_script( 'comment-reply' );
	}
}
add_action( 'wp_enqueue_scripts', 'megla_scripts' );

/*
 * This theme styles the visual editor to resemble the theme style,
 * specifically font, colors, and column width.
*/
add_editor_style( array(megla_fonts_url() ) );
